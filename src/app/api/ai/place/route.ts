...
export async function POST(req: NextRequest) {
  try {
    const { imageDataUrl, maskDataUrl, furnitureName, furnitureImageUrl, clickX, clickY } = await req.json()
    ...
    if (process.env.FAL_KEY) {
      ...
      try {
        const resultUrl = await runFluxFill({
          imageDataUrl,
          maskDataUrl,   // generated by browser canvas, no server Sharp needed
          furnitureName,
          cx,
          cy,
          wallNote,
