...
async function generateMockup(productDataUrl: string, productName: string, scene: (typeof SCENES)[number]): Promise<string> {
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (mockups.every(m => !m.imageUrl)) throw new Error('Hiçbir mockup üretilemedi')
